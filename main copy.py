...
# Configuration
directory_path = r'C:\Users\Amir\Desktop\PJ\MonoMicroPJ\xwiki-platform-xwiki-platform-10.8.3'
...
def parse_java_file(file_path):
    ...
    try:
        ...
        if not os.path.exists(file_path):
            print(f"File does not exist according to os.path.exists: {file_path}")
            return None, None
        ...
        return javalang.parse.parse(java_code), java_code
...
def extract_lexical_information(java_tree):
    ...
    for path, node in java_tree:
        try:
            ...
            print(f"Skipping node due to missing attribute: {e}")
        except Exception as e:
...
def extract_source_code_statements(node, class_info):
    ...
    # Correct handling for nodes with declarators
    elif isinstance(node, javalang.parser.tree.ConstantDeclaration):
        for declarator in node.declarators:
            class_info['SCS_ConstantDeclaration'].append(declarator.name)
    elif isinstance(node, javalang.parser.tree.VariableDeclaration):
        for declarator in node.declarators:
            class_info['SCS_VariableDeclaration'].append(declarator.name)
    elif isinstance(node, javalang.parser.tree.LocalVariableDeclaration):
        for declarator in node.declarators:
            class_info['SCS_LocalVariableDeclaration'].append(declarator.name)
        ...
        if node.return_type:
            ...
            class_info['SCS_MethodDeclaration'].append(return_type)
        ...
        if 'test' not in root.lower():
            for file in files:
                if file.endswith('.java'):
                    ...
                    try:
                        java_tree, java_code = parse_java_file(file_path)
                        class_package = java_tree.package.name if java_tree.package else "default"
                        extract_class_info(java_tree, class_package, variable_types)  # Remove i parameter
                    except Exception as e:
...
def get_class_id(class_name, package):
    for c in all_classes:
        if c[1] == class_name and c[2] == package:
            return c[0]
    return None

def extract_class_info(java_tree, class_package, variable_types):  # Remove i parameter
    for _, node in java_tree:
        if isinstance(node, javalang.parser.tree.ClassDeclaration):
            i = len(all_classes) + 1  # Unique ID based on current length
            ...
            for file_name in files:
                if file_name.endswith('.java'):
                    ...
                    try:
                        ...
                        all_class_info[file_name] = class_info
                    except Exception as e:
...
def process_trace_file_in_batches(trace_file_path, all_classes, batch_size=10000):
    # Initialize the co-occurrence matrix
    n = len(all_classes)
    co_occurrence_matrix = np.zeros((n, n), dtype=np.int32)

    # Open the trace file and process in batches
    with open(trace_file_path, 'r') as file:
        ...
        for line in file:
            ...
            if len(parts) < 2:
                continue
            class_names = [re.search(r'[\w\.]+\.(\w+)', element) for element in parts[1:]]
            class_names = [match.group(1) for match in class_names if match]
            class_ids = [next((c[0] for c in all_classes if c[1] == name), None) for name in class_names]
            class_ids = [cid for cid in class_ids if cid is not None]

            # Process the batch of class_ids
            batch.append(class_ids)
            if len(batch) >= batch_size:
                # Update the matrix for the batch
                for class_ids in batch:
                    for i in range(len(class_ids) - 1):
                        current_id = class_ids[i]
                        next_id = class_ids[i + 1]
                        if current_id != next_id:
                            co_occurrence_matrix[current_id - 1][next_id - 1] += 1
                            co_occurrence_matrix[next_id - 1][current_id - 1] += 1
    ...
    class_co_occurrences_df = pd.DataFrame(co_occurrence_matrix, index=[c[0] for c in all_classes], columns=[c[0] for c in all_classes])
    class_co_occurrences_matrix = np.where(class_co_occurrences_df == 0, 1, class_co_occurrences_df.values)
    ...
    return co_occurrence_matrix, co_occurrence_matrix_serializable


from concurrent.futures import ThreadPoolExecutor, as_completed

def calculate_similarity_for_pair(i, j, module1, module2, coefficients):
    if i <= j:
        total_similarity_ij = sum([coefficients[category] * calculate_similarity(module1, module2, category) for category in coefficients])
        return i, j, total_similarity_ij

def compute_total_similarity_parallel(new_lexical_info, coefficients):
    ...
    with ThreadPoolExecutor() as executor:
        ...
        for i, module1 in enumerate(new_lexical_info):
            for j, module2 in enumerate(new_lexical_info):
                future = executor.submit(calculate_similarity_for_pair, i, j, module1, module2, coefficients)
        ...
        for future in as_completed(future_to_pair):
            ...
            if result:
                total_similarity[result[0], result[1]] = result[2]
                total_similarity[result[1], result[0]] = result[2]

    return total_similarity



def precompute_comment_embeddings(new_lexical_info, model):
    all_comments = []
    module_indices = []
    current_idx = 0
    module_ids = list(new_lexical_info.keys())
    for module_id in module_ids:
        comments = filter_out_unwanted_comments(new_lexical_info[module_id]['CO'])
        all_comments.extend(comments)
        module_indices.append((current_idx, current_idx + len(comments)))
        current_idx += len(comments)
    
    if not all_comments:
        return None, module_ids
    
    with torch.no_grad():
        embeddings = model.encode(all_comments, convert_to_tensor=True, batch_size=128)
    
    avg_embeddings = []
    for start, end in module_indices:
        if start == end:
            avg = torch.zeros(embeddings.shape[1], device=embeddings.device)
        else:
            avg = torch.mean(embeddings[start:end], dim=0)
        avg_embeddings.append(avg)
    
    return torch.stack(avg_embeddings), module_ids

def compute_co_similarity(avg_embeddings):
    return util.pytorch_cos_sim(avg_embeddings, avg_embeddings).cpu().numpy()

def compute_category_similarity(args):
    i, j, cat_elements_i, cat_elements_j = args
    if not cat_elements_i or not cat_elements_j:
        return (i, j, 0.0)
    distances = process.cdist(cat_elements_i, cat_elements_j, scorer=Levenshtein.distance, workers=mp.cpu_count())
    lens_i = np.array([len(e) for e in cat_elements_i])[:, np.newaxis]
    lens_j = np.array([len(e) for e in cat_elements_j])[np.newaxis, :]
    max_lens = np.maximum(lens_i, lens_j)
    max_lens[max_lens == 0] = 1
    similarities = 1 - (distances / max_lens)
    return (i, j, similarities.mean())

def compute_scs_similarity(args):
    i, j, doc_i, doc_j = args
    if not doc_i and not doc_j:
        return (i, j, 1.0)
    if not doc_i or not doc_j:
        return (i, j, 0.0)
    return (i, j, fuzz.ratio(doc_i, doc_j) / 100.0)

def compute_total_similarity_optimized(new_lexical_info, coefficients):
    avg_embeddings, module_ids = precompute_comment_embeddings(new_lexical_info, model)
    n = len(module_ids)
    total_similarity = np.zeros((n, n))
    
    # Preprocess data for all categories
    non_scs_categories = [cat for cat in coefficients if cat != 'CO' and not cat.startswith('SCS')]
    scs_categories = [cat for cat in coefficients if cat.startswith('SCS')]
    
    # Preprocess non-SCS categories
    non_scs_data = {}
    for cat in non_scs_categories:
        elements_list = []
        for module_id in module_ids:
            elements = [re.sub(r'\b(byte|short|int|long|float|double|boolean|char|string|list|map|set|arraylist|hashmap|hashset|integer)\b', '', str(e).lower()).strip() 
                        for e in new_lexical_info[module_id][cat] if e]
            elements_list.append(elements)
        non_scs_data[cat] = elements_list
    
    # Preprocess SCS categories
    scs_docs = {}
    for cat in scs_categories:
        docs = []
        for module_id in module_ids:
            elements = new_lexical_info[module_id][cat]
            docs.append(" ".join(map(str, elements)))
        scs_docs[cat] = docs
    
    # Compute CO similarity
    if avg_embeddings is not None:
        co_sim = compute_co_similarity(avg_embeddings)
        total_similarity += coefficients['CO'] * co_sim
    
    # Compute non-SCS categories in parallel
    with ProcessPoolExecutor() as executor:
        for cat in non_scs_categories:
            elements_list = non_scs_data[cat]
            tasks = [(i, j, elements_list[i], elements_list[j]) 
                     for i in range(n) for j in range(i, n)]
            cat_sim = np.zeros((n, n))
            
            futures = [executor.submit(compute_category_similarity, task) for task in tasks]
            for future in futures:
                i, j, sim = future.result()
                cat_sim[i, j] = sim
                cat_sim[j, i] = sim
            
            total_similarity += coefficients[cat] * cat_sim
    
    # Compute SCS categories in parallel
    with ProcessPoolExecutor() as executor:
        for cat in scs_categories:
            docs = scs_docs[cat]
            tasks = [(i, j, docs[i], docs[j]) for i in range(n) for j in range(i, n)]
            cat_sim = np.zeros((n, n))
            
            futures = [executor.submit(compute_scs_similarity, task) for task in tasks]
            for future in futures:
                i, j, sim = future.result()
                cat_sim[i, j] = sim
                cat_sim[j, i] = sim
...
if __name__ == '__main__':
    ...
    # total_similarity = compute_total_similarity_parallel(new_lexical_info, coefficients)
    total_similarity = compute_total_similarity_optimized(new_lexical_info, coefficients)
    
    with open('coupling1.json', 'w') as file:
            json.dump({
    ...
    with open('coupling2.json', 'w') as file:
            json.dump({
                'conceptual_coupling_matrix': total_similarity.tolist(),
    ...
    co_occurrence_matrix, co_occurrence_matrix_serializable = process_trace_file_in_batches(traces_file_path, all_classes)
